...
public class ChatServer extends Thread {
    ...
    public void startServer() {
        try {
            ...
            LOG.info("Chat server started @ " + addr.getHostName() + "(Hostname)/" + addr.getHostAddress() + "(Hostadresse):" + PORT + "(Port)");
    ...
    public void stopServer() {
        ...
        for (int i = 0; i < this.clients.size(); i++) {
            ((ChatThread) this.clients.get(i)).out.print("none~.~server~.~none~.~!connectionClosed");
        }
        closeServer();
    }
    ...
    public boolean isBanned(String ip) {
        ...
        try {
            ...
            if (ms.getResult().length > 0) {
                ...
                if (time >= (System.currentTimeMillis() / 1000) - 86400) { // 24h
                    banned = true;
                }
            }
        } catch (Exception ex) {
    ...
    private class ChatThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                login();
                out.println("connected to " + name + " @ " + clientAddr + ":" + clientPort);
                String message;
                ...
                out.println("Disconnected from " + clientAddr + ":" + clientPort + " : " + name);
            }
        }
            ...
            if (command[0].equals("shutdown") && rights >= RIGHT_ADMIN) {
                ...
                if (command.length == 2) {
                    if (existChatUser(command[1])) {
                        // Rechte des gegenuebers pruefen
                ...
                try {
                    client.setSoTimeout(0);//Timeout to 12h - 1000 * 60 * 60 * 12
